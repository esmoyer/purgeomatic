...
def purge(movie):
    ...
    try:
        ...
        # Check if the movie has the "overseerr" tag
        movie_tags = radarr.get("tags", [])
        if "overseerr" in movie_tags:
            # Check if the movie was downloaded over 180 days ago
            download_date = datetime.strptime(movie["added_at"], "%Y-%m-%d %H:%M:%S")
            days_since_download = (datetime.now() - download_date).days
            if days_since_download > 180:
...
# Your existing code for fetching movies from Tautulli remains unchanged
